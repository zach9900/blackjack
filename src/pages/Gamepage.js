...
function Gamepage() {
  ...
  const countCards = (hand) => {
    ...
    return {
      cardsCount: hand
        .map(({ suit, number, imgPath }) => {
          return number;
        })
        .sort((a, b) => b - a)
        .map((value) => {
          if (value === 1 && res + 11 <= 21) res += 11;
          else res += value;

          return res;
        })[hand.length - 1],
      isBusted: res <= 21 ? false : true,
    };
  };
  ...
  const isPlayerWon = () => {
    ...
    return (
      ...
      (countCards(playerHand).cardsCount >= countCards(dealerHand).cardsCount &&
        !countCards(playerHand).isBusted)
    );
  };
  ...
  const getEndPrompt = (targetHand) => {
    let prompt = "";
    if (isBlackJack(targetHand)) prompt = "BlackJack";
    else if (countCards(targetHand).cardsCount > 21) prompt = "Bust";
  ...
  return (
    <Box sx={styles.box}>
      ...
      {screenState === "placeBet" && (
        <img
          src={require("../assets/btnPlaceBet.png")}
          alt=""
          style={styles.btnPlaceBet}
          onClick={() => {
            drawToPlayer();
            setScreenState("gameStart");
          }}
        />
      )}

      {screenState === "gameStart" && (
        <img
          src={require("../assets/btnHit.png")}
          alt=""
          style={styles.btnHit}
          onClick={() => {
            drawToPlayer();
      ...
      {screenState === "gameStart" && (
        <img
          src={require("../assets/btnStand.png")}
          alt=""
          style={styles.btnStand}
          onClick={() => {
            setScreenState("dealersTurn");
          }}
        />
      ...
      {screenState === "gameStart" && playerHand.length <= 2 && (
        <img
          src={require("../assets/btnDouble.png")}
          alt=""
          style={styles.btnDouble}
          onClick={() => {
            console.log("money is doubled");
            drawToPlayer();
            setScreenState("dealersTurn");
            setTimeout(() => {}, 500);
          }}
      ...
      {(screenState === "dealersTurn" || screenState === "gameEnded") && (
        <Typography sx={styles.dealersCounter} variant="h2">
          {getEndPrompt(dealerHand)}
        </Typography>
      )}

      {screenState !== "placeBet" && (
        <Typography sx={styles.playersCounter} variant="h2">
      ...
      {screenState === "gameEnded" && (
        <Typography sx={styles.results} variant="h2">
          {isPlayerWon()
            ? `player wins ${
                isBlackJack(playerHand)
                  ? "BlackJack"
                  : countCards(playerHand).cardsCount
              }`
            : `dealer wins ${
                isBlackJack(dealerHand)
                  ? "BlackJack"
                  : countCards(dealerHand).cardsCount
              }`}
        </Typography>
      )}
